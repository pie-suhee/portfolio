...
(function (factory) {
  ...
  function getTouchPageX(e) {
    e = e.originalEvent || e;
    this.zoom = $("#wrap").css("zoom");
    return (
      (window.event &&
        window.event.changedTouches &&
        event.changedTouches[0].pageX) / zoom || e.pageX / zoom
    );
  }
  ...
  function getTouchPageY(e) {
    ...
    return (
      (window.event &&
        window.event.changedTouches &&
        event.changedTouches[0].pageY) / zoom || e.pageY / zoom
    );
  }
  ...
  function trigger(el, name, originalEvent, arg) {
    ...
    var win =
      el[0].ownerDocument.defaultView || el[0].ownerDocument.parentWindow;
  ...
  Dragging.prototype.start = function (parent, el, e, handle) {
    ...
    this.origin.offset.x =
      rect.left + (window.scrollX || window.pageXOffset) - this.origin.x;
    this.origin.offset.y =
      rect.top + (window.scrollY || window.pageYOffset) - this.origin.y;
  ...
  Dragging.prototype.move = function (e) {
    ...
    if (e.type !== "scroll") {
      ...
      var clientX =
          e.clientX ||
          (e.originalEvent && e.originalEvent.clientX) ||
          (window.event &&
            window.event.touches &&
      ...
      var pageX =
          this.lastX +
          ((window.scrollX || window.pageXOffset) - this.origin.scrollX),
        pageY =
          this.lastY +
          ((window.scrollY || window.pageYOffset) - this.origin.scrollY);
    }

    // border scrolling only for root window
    if (e.view !== win && e.view && e.view.frameElement) {
      var bottom =
        (pageY - (window.scrollY || window.pageYOffset) - window.innerHeight) *
        -1;
      var bottomReached =
  ...
  Dragging.prototype.adjustPlacement = function (e) {
    ...
    this.origin.x =
      rect.left + (window.scrollX || window.pageXOffset) - this.origin.offset.x;
    this.origin.y =
      rect.top + (window.scrollY || window.pageYOffset) - this.origin.offset.y;
  ...
  Droppable.prototype.activate = function (e) {
    this.accept =
      dragging.parent.opts.connectWith &&
      this.el.is(dragging.parent.opts.connectWith);
    ...
    if (this.opts.activeClass) this.el.addClass(this.opts.activeClass);
  ...
  Droppable.prototype.drop = function (e) {
    ...
    var el = dragging.el;
    var handler =
      typeof this.opts.receiveHandler === "function" &&
      this.opts.receiveHandler;
    var evtObj = { item: el };
    var clone = null;
    if (dragging.handle) {
  ...
  Sortable.prototype.create = function () {
    ...
    this.observer = new SelectorObserver(
      this.el[0],
      this.opts.items,
      function () {},
      function () {
        if (
          this === self.placeholder[0] ||
          (dragging.el && this === dragging.el[0])
        ) {
          return;
        }
        var item = $(this);
  ...
  Sortable.prototype.indexOf = function (el) {
    return this.el
      .find(this.opts.items + ", #" + this.placeholder.attr("id"))
      .index(el);
  };
  ...
  Sortable.prototype.start = function (e) {
    ...
    if (this.index !== null) {
      ...
      var height = dragging.el.outerHeight
        ? dragging.el.outerHeight()
        : dragging.el.height();
  ...
  Sortable.prototype.diverted = function (e) {
    ...
    dragging.placeholder = this.placeholder;
    this.direction =
      this.indexOf(this.placeholder.show()) < this.indexOf(child)
        ? "down"
        : "up";
      ...
      this.each(function () {
        ...
        if (typeof opts === "string") {
          ...
          switch (opts) {
            ...
            case "option":
              // set
              if (value !== undefined) instance.opts[name] = value;
              else if (typeof name === "object")
                instance.opts = $.extend(instance.opts, name);
              // get
              else if (name) result.push(instance.opts[name]);
              else result.push(instance.opts);
              break;
            // case 'serialize':
  ...
  $.fn.draggable = generic(Draggable, "draggable", {
    cancel: "input, textarea, button, select, option, li",
    connectWith: false,
    cursor: "auto",
    disabled: false,
    handle: false,
